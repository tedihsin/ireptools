...
output:
  pdf_document:
    ...
    pandoc_args: ['-r', 'markdown-auto_identifiers', '-V', 'indent=true'] 
...
# setupチャンクに更新があれば、他チャンク実行時に再読み込みされる
...
if (!is.null(knitr::opts_knit$get("rmarkdown.pandoc.to"))) {
  if (knitr::opts_knit$get("rmarkdown.pandoc.to") %in% c("beamer", "latex")) {
    ...
    # デフォルトのフォントを指定する
    knitr::opts_chunk$set(dev = "cairo_pdf", dev.args = list(family = "NotoSansCJK-Light"))
  }
} else {
  # PDF以外の時は基本的にPNGとする
  # PNGの場合、必要に応じて解像度の指定を行う
  knitr::opts_chunk$set(dpi = 300, dev = "png", dev.args = list(type = "cairo-png", family = "NotoSansCJK-Light"))
}
```
